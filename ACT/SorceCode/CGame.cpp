...
bool CGame::Create()
{
	...
	if (NoCreateInstance != true) {
		...
		m_MasterWorkDC = CreateCompatibleDC(m_pGameWnd->hScreenDC);						//作業用DC2の作成
		m_MasterWorkBmp = CreateCompatibleBitmap(m_pGameWnd->hScreenDC, WND_W, WND_H);	//作業用BITMAP2の作成
		SelectObject(m_MasterWorkDC, m_MasterWorkBmp);									//DC2とBITMAP2を紐づけ
	}
	...
	////ステージメイン画像のインスタンス生成
	//m_pStageMainImg = new CImage(m_pGameWnd->hScreenDC, m_hMemDC, m_hWorkDC, m_hWorkDC2);
	////ステージ背景1画像のインスタンス生成
	//m_pStageBG_1Img = new CImage(m_pGameWnd->hScreenDC, m_hMemDC, m_hWorkDC, m_hWorkDC2);
	////ステージ背景2画像のインスタンス生成
	//m_pStageBG_2Img = new CImage(m_pGameWnd->hScreenDC, m_hMemDC, m_hWorkDC, m_hWorkDC2);

	//プレイヤーの読み込み.
	if (m_pPlayerImg->LoadBmp("Data\\Image\\Player.bmp") == false)	return false;
	//エネミーの読み込み.
	if (m_pEnemyImg->LoadBmp("Data\\Image\\Enemy.bmp") == false)	return false;

	////ステージメインの読み込み.
	//if (m_pStageMainImg->LoadBmp("Data\\MapImage\\StageMain.bmp") == false)	return false;
	////ステージ背景1の読み込み.
	//if (m_pStageBG_1Img->LoadBmp("Data\\MapImage\\BackGround_1.bmp") == false)	return false;
	////ステージ背景2の読み込み.
	//if (m_pStageBG_2Img->LoadBmp("Data\\MapImage\\BackGround_1.bmp") == false)	return false;
	...
	m_pStage = new CStage();
	//画像の設定
	m_pStage->SetImage(m_pStageMainImg, m_pStageBG_1Img, m_pStageBG_2Img);
	//マップデータの読み込み
...
void CGame::Destroy()
{
	...
	SAFE_DELETE(m_pStageMainImg);
	SAFE_DELETE(m_pStageBG_1Img);
	SAFE_DELETE(m_pStageBG_2Img);
